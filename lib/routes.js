...
module.exports = function MongooseRestRouter(base, router, options) {
    ...
    router.get(reGet, function (req, res, next) {
        res.contentType('json');
        var single = req.query.single || (_u.compact(Array.isArray(req.params) ? req.params : _u.values(req.params)).length + (req.params[2] ? req.params[2].split('/').length - 1 : 0)) % 2 == 0;
        var Model = m(req.params[0]);
        var find = Model.findOne(idObj(req.params[1]));
        populate(find, req.query);

        var stuff = Object.keys(req.params).length > 1 ? req.params[2] : null;
        if (stuff) {
    ...
    router.put(reGet, function (req, res, next) {
        res.contentType('json');
        var single = req.query.single;
        var stuff = [].concat(Array.isArray(req.params) ? req.params : _u.values(req.params));
        stuff.shift();
